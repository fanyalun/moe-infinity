...
class OffloadEngine(object):
    ...
    def _patch_qwen3_vl_vision(self, model):
        """Monkey-patch fast_pos_embed_interpolate to fix device
        mismatch.  The original captures
        ``device = self.pos_embed.weight.device`` before the offload
        hook fires, so idx/weight tensors end up on CPU while the
        embedding output lands on GPU.  The patched version creates
        tensors on CPU first, then derives the device from the
        embedding output."""
        import types
        ...
        if visual is None or not hasattr(visual, "fast_pos_embed_interpolate"):
            return
        ...
        def _fixed_fast_pos_embed_interpolate(self, grid_thw):
            ...
            # build on CPU; self.pos_embed() triggers the offload
            # hook which may move weight to GPU
            idx_tensor = torch.tensor(
                idx_list, dtype=torch.long
            )
            ...
            patch_pos_embeds = torch.cat(
                patch_pos_embeds_permute
            )
            return patch_pos_embeds

        visual.fast_pos_embed_interpolate = types.MethodType(
            _fixed_fast_pos_embed_interpolate, visual
        )
