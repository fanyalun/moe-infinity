...
- **数据集**: RealWorldQA (20 个样本，random_state=42)
- **生成参数**: max_new_tokens=50, do_sample=False
...
这将运行 benchmark 并将结果保存到 `./results/moe_infinity_comparison/`。

### 3. 运行 Offload Baseline Benchmark（如果还没有运行）

```bash
cd /home/fanya/qwen3_vl_moe_test
python run_inference_offload.py \
    --model-path /data1/fanya/models/Qwen/Qwen3-VL-30B-A3B-Instruct \
    --data-dir /data1/fanya/datasets/realworldqa/realworldQA.tsv \
...
python compare_performance.py \
    --baseline /home/fanya/qwen3_vl_moe_test/results/baseline_comparison/metrics.json \
    --moeinf ./results/moe_infinity_comparison/metrics.json \
...
- Baseline 实现: `/home/fanya/qwen3_vl_moe_test/run_inference_offload.py`
